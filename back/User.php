<?php
    ...
    function createUser($uid, $fname, $lname, $id) {
        ...
        if($result_user_exists) {
            $_SESSION["message"] = "Error! User account already exists";
            header("Location: ../");
            $mysqli.close();
            exit;
        }
        ...
        $priv = 0;
        $query_add_user->bind_param($uid, $fname, $lname, $id, $priv, $priv, $priv);
        $result_add_user = $query_add_user->execute();

        if(!$result_sessions) {
            $_SESSION["message"] = "Error! Could not add user.";
            header("Location: ../");
            $mysqli.close();
            exit;
        }
